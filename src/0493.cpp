...
// Fenwick Tree - compression with set
...
// Fenwick Tree - compression with sorting
...
public:
    int mergeSort(vector<int> &nums, int l, int r)
    {
        ...
        int i = l, j = m + 1, k = m + 1, index = 0;
        vector<int> temp(r - l + 1);

        while (i <= m)
        {
            while (k <= r && nums[i] > 2ll * nums[k])
                ++k;
            c += k - (m + 1);
        ...
        while (j <= r)
            temp[index++] = nums[j++];
        copy(begin(temp), end(temp), begin(nums) + l);
